 ...
 const NuevoPassword = () => {
  ...
  useEffect(() => {
    const comprobarToken = async () => {
      try {
        await clienteAxios(`/mecanico/restablecer-pass/${ token }`)
        setAlerta({
  ...
  const handleSubmit = async (e) => {
    ...
    if ( password.length < 6) {
      setAlerta({
        msg: 'La contraseña debe tener un mínimo 6 caracteres.',
        error: true
      })
      return
    }
    ...
    try {
      const url = `/mecanico/restablecer-pass/${ token }`
  ...
  return (
    <>
      ...
      <div className="mt:20 md:mt-5 shadow-lg px-5 py-10 rounded-xl bg-white">
        ...
        {passwordModificado && 
            <Link className="block text-center my-5 text-gray-600"
              to="/">Iniciar sesión
            </Link>
        }
